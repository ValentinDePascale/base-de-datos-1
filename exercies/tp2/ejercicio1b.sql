...
-- Ejercicio 2
SELECT DISTINCT socios.nombre AS nombre, barcos.nombre AS barco, barcos.cuota
FROM Socios
JOIN Barcos ON socios.id_socio = barcos.id_socio
WHERE socios.nombre = "Juan Pérez";

-- Ejercicio 3
SELECT barcos.matricula, COUNT(salidas.matricula) AS salidas
FROM Salidas
JOIN Barcos ON salidas.matricula = barcos.matricula
WHERE barcos.matricula = 'ABC123';

-- Ejercicio 4
SELECT socios.nombre, barcos.cuota
FROM Socios
JOIN Barcos ON socios.id_socio = barcos.id_socio
...
-- Ejercicio 6
SELECT Salidas.patron_nombre AS patron, Salidas.patron_direccion AS direccion
FROM Salidas
JOIN Barcos ON Salidas.matricula = Barcos.matricula
